...
(() => {
  ...
  // src/styles-compact.js
  var COMPACT_READER_STYLES = `
  /*
...
`;
  ...
  function installShell(ctx2) {
    ...
    const handleBokounScroll = (...args) => ctx2.handleBokounScroll(...args);
  ...
  function installScrollState(ctx2) {
    const {
  ...
  function relativeActivityFromTimestamp(datetime, currentTime = Date.now()) {
    if (!datetime) return "";
    const timestamp = Date.parse(datetime);
    if (!Number.isFinite(timestamp)) return "";
    const seconds = Math.max(0, Math.round((currentTime - timestamp) / 1e3));
  ...
  function boardModelFromSvelteRoots(roots, pageHref, { sanitize } = {}) {
